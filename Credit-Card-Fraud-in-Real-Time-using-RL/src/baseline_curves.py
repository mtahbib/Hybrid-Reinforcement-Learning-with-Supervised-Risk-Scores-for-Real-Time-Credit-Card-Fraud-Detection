# run_baselines.py  — one-file baseline trainer (LR, RF, XGBoost-GPU) + PR/ROC plotting
import json, joblib
...
def plot_precision_recall(y_true, proba_dict, scores_dict, path):
    plt.figure(dpi=150)
    for name, proba in proba_dict.items():
        p, r, _ = precision_recall_curve(y_true, proba)
        auprc = scores_dict[name]["AUPRC"]
        plt.step(r, p, where="post", linewidth=2, label=f"{name} (AUPRC={auprc:.3f})")
    plt.xlabel("Recall")
...
def plot_roc(y_true, proba_dict, scores_dict, path):
    plt.figure(dpi=150)
    for name, proba in proba_dict.items():
        fpr, tpr, _ = roc_curve(y_true, proba)
        auroc = scores_dict[name]["AUROC"]
...
def main():
    ...
    proba_dict["RandomForest"] = proba_rf
    ...
    results["XGBoost"] = eval_scores(yte, proba_xgb)
    proba_dict["XGBoost"] = proba_xgb
    ...
    plot_precision_recall(
        ...
        scores_dict=results,
        path=str(FIGS / "pr_curve_test.png"),
    )
    plot_roc(
        y_true=yte,
        proba_dict=proba_dict,
        scores_dict=results,
        path=str(FIGS / "roc_curve_test.png"),
    )
    print("Saved figures to:", str(FIGS.resolve()))
